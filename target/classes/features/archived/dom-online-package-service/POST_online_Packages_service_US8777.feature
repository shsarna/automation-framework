...
#
# @SP_67_US_8777_Test_11432 @DDIBA-11434 @DDIBA-11433 @DDIBA-11435
...
#  #TestID:DDIBA-11432
  ...
  #-----------------dom online package db and OA-VehicleBackend db need to be clean to check validities---------------
    ...
    And BA-platform publishes the notification message '/testData/MessagesTOPublish/dom_content_service/disclaimer.txt' in the queues bind to PSE_exchange
...
#TestID:DDIBA-11434
   # -----------------------------------------------------------GET onlinePackageservice with specific value in url------------------------
  ...
  Scenario Outline: GET onlinepackages/{id} response has the 'state of construction' alongside with the other n-tuple values, if available
    ...
    Examples:
      ...
      | /testData   |
#
#    #---------------------------------------*********************************End of above scenario***************************************************************
#
#    #---------------------------------------*********************************End of above scenario***************************************************************

  #TestID:DDIBA-11435
  #-------------DOM Online Package Service takes new validity 'state of construction' into consideration when creating the online package resource, when triggered by a message from XXXML Import Service-----------------------
  Scenario Outline: DOM Online Package Service takes new validity 'state of construction' into consideration when creating the online package resource, when triggered by a message from XXXML Import Service
    ...
    And wait for sync status '2' and core status '2'
     ...
     #-------------------- Validating Dom online Package db --------------------------
    ...
    Then the database table 'dom_online_package.dom_online_package_resource' should contain a row with values in '<test_folder>/dom-online-package-service/POST_onlinepackages/then/dom-online-packages_db/dom_online_package_resource_id2.json'

  #-------------------- Validating Dom content  db---------------------------------------------------------------------------------------------
    Then the database table 'dom_content.dom_database_resource' should contain a row with values in '<test_folder>/dom-online-package-service/POST_onlinepackages/then/oa-vehicle-backend_db/dom-content-database-resource_id2.json'
 ...
 #-------------------- Validating Dom content  db---------------------------------------------------------------------------------------------
    ...
    #-------------------- Validating downloading from COS and validating data for instr.db form---------------------------------------------------------------------------------------------
  ...
  #   #TestID:DDIBA-11433
#   # -----------------------------------------------------------GET onlinePackageservice with unspecific value in url------------------------

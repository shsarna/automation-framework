@Regression_Set1 @RegressionAll
@SP72_US_10575_Test_12606
Feature: Provide import/sync details in message pushed by XXXML Import Service

  @redeployOaVeBackendDb
  Scenario Outline: Provide import/sync details in message pushed by XXXML Import Service (Import: success , dom content synchronization: business error and dom ui synchronization :success)
    ...
    Then the database table 'xxxml_v10_import.xxxml_import' should contain a row with values in '<test_folder>/then/import-db/xxxml_import_dom_content_business_error.json'
